...
Crawl the site's key pages to check technical health. Use headless Chrome for JavaScript-rendered pages (recommended for SPA/SSR sites), the firecrawl skill if available, or WebFetch as a fallback.

**Page Fetching Options:**
- **Recommended**: Use `python3 "$SKILL_SCRIPTS/chrome_audit.py" --url <URL> --output /tmp/audit.json` — uses MCP CDP for full JavaScript rendering (via @modelcontextprotocol/server-chrome-devtools)
- **For SPA/SSR Detection with MCP**: Run `python3 "$SKILL_SCRIPTS/detect_js.py" --use-mcp --url <URL>` — uses MCP to fetch rendered HTML for accurate framework detection
- **Fallback**: Use WebFetch if Chrome/MCP unavailable
...
**⚡ Speed note**: Fetch all 5 pages using parallel bash calls in a single
